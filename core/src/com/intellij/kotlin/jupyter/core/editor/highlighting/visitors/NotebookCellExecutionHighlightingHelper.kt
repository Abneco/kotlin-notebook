...
class NotebookCellExecutionHighlightingHelper(
  ...
  private val notebookFile: BackedNotebookVirtualFile
    ...
    init {
        val parentDisposable = NotebookHighlightingService.Companion.getInstance(project)
        project.messageBus.connect(parentDisposable).subscribe(
          NotebookSessionEventListener.Companion.TOPIC,
    ...
    private val jupyterNotebookSession get() = JupyterRuntimeService.Companion.getInstance(project).getNotebookSession(notebookFile)
    ...
    private fun queueCurrentCell(project: Project, file: BackedNotebookVirtualFile, index: Int) {
        val highlightingManager = NotebookHighlightingService.Companion.getForFile(project, file)
        // add current cell
