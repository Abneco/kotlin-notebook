...
internal class NotebookSessionNoSuspensionEnvironmentProvider(
    ...
    private fun retrieveVariablesStateReference(virtualMachine: VirtualMachineProxy): ObjectReference? {
        ...
        val sharedContextReference = notebookReference.getFieldValueByName("sharedReplContext")
                as? ObjectReference ?: return null

        val evaluatorImpl = sharedContextReference.getFieldValueByName("evaluator") as ObjectReference

        return evaluatorImpl.getFieldValueByName("variablesHolder") as? ObjectReference
    }
    ...
    private fun retrieveValueFromVariableState(variablesState: Value, variableName: String): Pair<ObjectReference?, Field> {
        ...
        val suitableFieldAccessor = scriptInstance.findFieldByName(variableName)
            ?: throw IllegalArgumentException("Could not find variable with name: $variableName")
    ...
    private fun retrieveMapEntryReferenceAndVariablesSize(variablesStateReference: ObjectReference): Pair<ObjectReference?, Int> {
        ...
        val currentSize = (variablesStateReference.getFieldValueByName("size") as? IntegerValue)?.value()
        if (currentSize == null) {
            throw IllegalArgumentException("Could not retrieve size from VariablesState")
        }
        return variablesStateReference.getFieldValueByName("head") as? ObjectReference to currentSize
    }
}