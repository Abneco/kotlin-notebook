...
import com.intellij.kotlin.jupyter.debug.util.shouldFocusOnVariablesToolWindow
...
class KotlinNotebookVarsToolWindow(
    ...
    internal val panelSetupData: KotlinNotebookVariablesToolWindowConfiguration
    ...
    private fun subscribeToEvents() {
        project.messageBus.connect(this).subscribe(
            JupyterEnvironmentUpdateListener.TOPIC,
            VariablesListener()
        )
    }
    ...
    private val shouldUpdateVariablesList: Boolean
        get() = when {
            project.isDisposed || !project.shouldShowNotebookVariables -> {
                showMessage(
                    KotlinNotebookDebugBundle.message("kotlin.jupyter.debug.node.default.message")
                )
                false
            }
            !notebookFile.debugFeaturesSupported(project) -> {
                showMessage(
                    KotlinNotebookDebugBundle.message("kotlin.jupyter.debug.node.not.enabled.message")
                )
                false
            }
            else -> true
    ...
    private fun removeClickListener() {
        ...
        if (variablesView != null && listener != null) {
            ListenerUtil.removeClickListener(variablesView.panel, listener)
        }
        clickListener = null
    }
